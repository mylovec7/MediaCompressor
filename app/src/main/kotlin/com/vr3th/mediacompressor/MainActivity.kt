...
class MainActivity : Activity() {
    ...
    // ---- Sovereign Obsidian Rose palette -----------------------------------
    private val bg = Color.parseColor("#07060B")          // Deepest Pure Obsidian (AMOLED)
    private val panel = Color.parseColor("#12101E")       // Clean Dark Crystal surface
    private val panelStroke = Color.parseColor("#2E2442") // sleek luminous hairline border
    private val activeCardBg = Color.parseColor("#1D132B") // Active Chamber (Rose Tint)
    private val pinkSoft = Color.parseColor("#FF59A2")    // Holographic Blossom
    private val pinkElectric = Color.parseColor("#FF1A85") // Cyber Blossom laser border / accent
    private val roseGold = Color.parseColor("#FF59A2")
    private val pastelPink = Color.parseColor("#FCE4EC")
    private val white = Color.parseColor("#FFFFFF")       // Pure Diamond White
    private val gray = Color.parseColor("#FCE4EC")        // Radiant Champagne Rose
    private val metaGray = Color.parseColor("#9893AA")    // Velvet Titanium Gray
    private val mint = Color.parseColor("#00FF9D")        // Hyper-Emerald Mint
    private val waveGradientStart = Color.parseColor("#FF5CA8") // progress-fill gradient start (per spec §4)
    ...
    private fun fileHeaderCard(name: String, metaLine: String, allowMultiple: Boolean, onReplaced: () -> Unit) {
        ...
        val repickBtn = TextView(this).apply {
            ...
            gravity = Gravity.CENTER
            typeface = Typeface.create("sans-serif-medium", Typeface.NORMAL)
            background = shape(Color.TRANSPARENT, pinkSoft, 20f)
    ...
    private fun qualityProfileUI(r: LinearLayout) {
        ...
        fun refresh() {
            cards.forEach { (card, p) ->
                ...
                card.background = shape(if (active) activeCardBg else panel, if (active) pinkElectric else panelStroke, 14f, if (active) 2 else 1)
    ...
    private fun runOperation(type: String, op: String) {
        ...
        // Back behavior changes with screen state: while processing, back
        // cancels the operation and returns to Feature Configuration; once a
        // result (or cancellation) is shown, back just returns there. Both
        // the "<" button and the system/gesture back invoke this same
        // indirection, so they always agree.
        var backAction: () -> Unit = {
            cancelled.set(true)
        ...
        fun enterProcessingMode() {
            ...
            val w = WaveProgressView(this, panel, waveGradientStart, pinkElectric).apply {
