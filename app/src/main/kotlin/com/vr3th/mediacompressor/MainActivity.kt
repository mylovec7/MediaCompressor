...
class MainActivity : Activity() {
    ...
    // ---- Sovereign Obsidian Rose palette (Hyper-Luminous Edition) ---------
    ...
    private val panelStroke = Color.parseColor("#3D3055") // crisp specular stroke
    private val activeCardBg = Color.parseColor("#221333") // Luminous Chamber (active card fill)
    private val pinkSoft = Color.parseColor("#FF59A2")    // Holographic Blossom
    private val pinkElectric = Color.parseColor("#FF2B8D") // Hyper-Electric Pink — laser border / accent
    private val cyberBlossom = Color.parseColor("#FF47A3") // Radiant Cyber Blossom — checkmarks, highlights
    private val neonRose = Color.parseColor("#FF007F")     // Neon Rose — deepest glow accent
    ...
    private fun runOperation(type: String, op: String) {
        ...
        fun enterProcessingMode() {
            ...
            val cancelBtn = button("[ ✕ CANCEL ]") { cancelled.set(true); workerThread?.interrupt() }
                .apply { background = shape(panel, cyberBlossom, 12f); setTextColor(cyberBlossom) }
