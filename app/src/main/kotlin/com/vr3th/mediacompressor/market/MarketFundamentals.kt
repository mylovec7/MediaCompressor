    ...
    private fun fetchConcept(cik: String, concept: String): Pair<Double, String?>? {
        ...
        return try {
            ...
            for (i in usd.length() - 1 downTo 0) {
                val entry = usd.optJSONObject(i) ?: continue
                if (entry.optString("form") == "10-K") { 
                    best = entry
                    break 
                }
            }
            
            val target = best ?: usd.optJSONObject(usd.length() - 1) ?: return null
            val value = target.optDouble("val", Double.NaN)
            if (value.isNaN()) null else {
                val fy = target.optString("fy", "")
                val fp = target.optString("fp", "")
