...
public final class VideoCompressor {
    ...
    VideoCompressor(MainActivity a) {
        this.a = a;
    }
    ...
    void run(Uri src, AtomicBoolean cancel) {
        ...
        try {
            ...
            if (!isSupportedInput(ext)) {
                throw new IOException("Format tidak termasuk daftar Video Compressor.");
            }
            ...
            if (local.length() <= 0) {
                throw new IOException("input could not be localized");
            }

            Meta m = readMeta(local);

            if (m.duration <= 0) {
                throw new IOException("video duration unavailable");
            }

            boolean sourceAudio = hasAudio(local);

            // If the source is already very low bitrate, transcoding is unlikely to help.
            if (original > 0 && m.videoBitrate > 0
                    && m.duration > 0
                    && m.videoBitrate < 260_000) {
            ...
            double[] factors = codec.equals(MimeTypes.VIDEO_H265)
                    ? new double[]{0.72, 0.60, 0.50}
                    : new double[]{0.68, 0.56, 0.46};
            ...
            for (double factor : factors) {
                ...
                long base = m.videoBitrate > 0
                        ? m.videoBitrate
                        : Math.max(320_000, (original * 8) / m.duration);
                ...
                /*
                 * FIX:
                 * VideoEngine.builder() membutuhkan int.
                 * target tetap dihitung sebagai long agar aman,
                 * lalu dibatasi ke range int sebelum dikirim.
                 */
                ...
                File out = new File(
                        a.outputDir(),
                        ".tmp_compress_" + System.nanoTime() + ".mp4"
                );

                candidates.add(out);

                String error = transform(
                        mediaItemFor(local),
                        out,
                        cancel,
                        targetBitrate,
                ...
                if (error != null || !out.exists() || out.length() <= 0) {
                    safe(out);
                    continue;
                }

                if (!hasValidVideo(out)
                        || (sourceAudio && !hasAudio(out))) {
                    safe(out);
                    continue;
                }
                ...
                if (resultDur <= 0
                        || Math.abs(resultDur - m.duration) > 1_500_000L) {
                    safe(out);
                    continue;
                }

                // NEVER accept an output larger than or equal to original.
                if (original <= 0 || out.length() >= original) {
                    safe(out);
                    continue;
                }
                ...
                c.file = out;
                c.size = out.length();
                c.bitrate = targetBitrate;
                c.codec = codec;

                // Prefer the highest-quality result that still achieves
                // a meaningful reduction.
                if (best == null || better(c, best, original)) {
                ...
                if (original > 0
                        && best.size <= Math.round(original * 0.82)) {
                    break;
                }
            }
            ...
            a.publishVideo(best.file);
            a.videoProgress(100);

            double saved =
                    (original - best.size) * 100.0 / original;
            ...
            a.videoFinished(
                    "> SMART COMPRESSION: COMPLETE\n"
                            ...
                            + "\n✓ "
                            + (
                            best.codec.equals(MimeTypes.VIDEO_H265)
                                    ? "H.265/HEVC"
                                    : "H.264/AVC"
                    )
                            + " HARDWARE PATH"
    ...
    private boolean better(
        ...
        boolean xMeaningful =
                x.size <= Math.round(original * 0.85);

        boolean yMeaningful =
                y.size <= Math.round(original * 0.85);

        if (xMeaningful != yMeaningful) {
            return xMeaningful;
        }

        // Higher bitrate = higher potential quality.
            ...
            for (int i = 0; i < ex.getTrackCount(); i++) {
                ...
                if (mime.startsWith("video/")) {
                    ...
                    if (fmt.containsKey(MediaFormat.KEY_BIT_RATE)) {
                        m.videoBitrate = Math.max(
                                m.videoBitrate,
                                fmt.getLong(MediaFormat.KEY_BIT_RATE)
                        );
                    }
                    ...
                    if (fmt.containsKey(MediaFormat.KEY_HEIGHT)) {
                        ...
                        && fmt.containsKey(MediaFormat.KEY_BIT_RATE)) {

                    m.audioBitrate = Math.max(
                            m.audioBitrate,
                            fmt.getLong(MediaFormat.KEY_BIT_RATE)
                    );
                }
    ...
    private String guessExtension(Uri src) {
        ...
        String x = extensionFromUri(src);

        return x.matches("\\.[a-z0-9]{1,8}")
                ? x
                : ".mp4";
    }
    ...
    private boolean isSupportedInput(String x) {
        ...
        return x.matches(
                "\\.(3g2|3gp|f4v|m4v|mkv|mov|mp4|qt|webm|m2ts|mts|ts|avi|flv|m2v|mpeg|mpg|mxf|ogv|rm|wmv|wtv|vob)$"
        );
    }
    ...
    private String transform(
        ...
        main.post(() -> {
            ...
            try {
                ...
                t.start(
                        new EditedMediaItem.Builder(item)
                                .build(),
                        out.getAbsolutePath()
                );
                ...
                error[0] =
                        e.getMessage() == null
                                ? "transformation could not start"
                                : e.getMessage();
    ...
    private String cleanError(String s) {
        ...
        if (l.contains("encoder")
            ...
            return "No compatible video encoder is available in this device.";
        }

        if (l.contains("decoder")) {

            return "No compatible decoder is available for this video.";
        }
