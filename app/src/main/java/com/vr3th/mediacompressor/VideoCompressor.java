...
/** Native-only adaptive compressor. Every accepted video is re-encoded. */
public final class VideoCompressor {
    ...
    private static final class Candidate{File file;long size;int bitrate;}
    ...
    void run(Uri src, AtomicBoolean cancel){
        ...
        try{
            ...
            for(int f:factors){
                ...
                NativeVideoEngine.Result nr=NativeVideoEngine.encodeFrames(a,local,out,0,m.duration,false,target,cancel);
                if(!out.exists()||out.length()<=0)continue;
                if(!validVideo(out))continue;
                if(original>0&&out.length()>=original)continue;
                Candidate c=new Candidate();c.file=out;c.size=out.length();c.bitrate=target;
                if(best==null||c.size<best.size){if(best!=null)safe(best.file);best=c;}
                if(original>0&&best.size<=Math.round(original*.55))break;
            }
            if(best==null)throw new IOException("No smaller encoded output was produced");
