...
public final class VideoCompressor {
    ...
    private String guessExtension(Uri src){
        ...
        return x.matches("\\.[a-z0-9]{1,8}")?x:".mp4";
    }

    private MediaItem mediaItemFor(File f){
        return MediaItem.fromUri(Uri.fromFile(f));
    }
    ...
    private String transform(MediaItem item,File out,AtomicBoolean cancel,long bitrate,String codec)throws Exception{
        ...
        main.post(()->{try{
            Transformer t=VideoEngine.builder(
            ...
            t.start(
                    new EditedMediaItem.Builder(item).build(),
                    out.getAbsolutePath()
            );
    ...
    private boolean hasValidVideo(File f){
        ...
        try{
            ...
            for(int i=0;i<ex.getTrackCount();i++){
                MediaFormat x=ex.getTrackFormat(i);
                String m=x.getString(MediaFormat.KEY_MIME);

                if(m!=null&&m.startsWith("video/")
                        &&x.containsKey(MediaFormat.KEY_DURATION)
                        &&x.getLong(MediaFormat.KEY_DURATION)>0)
                    return true;
            }
    ...
    private long durationUs(File f){
        ...
        try{
            ...
            for(int i=0;i<ex.getTrackCount();i++){
                ...
                if(x.containsKey(MediaFormat.KEY_DURATION))
                    d=Math.max(d,x.getLong(MediaFormat.KEY_DURATION));
            }
    ...
    private String cleanError(String s){
        ...
        if(l.contains("encoder")&&l.contains("available"))
