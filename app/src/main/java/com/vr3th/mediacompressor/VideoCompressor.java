    ...
    void run(Uri src, AtomicBoolean cancel){
        ...
        try{
            ...
            if(error!=null || !out.exists() || out.length()<=0){
                safe(out);
                // Only legacy/unsupported inputs reach the heavier fallback.
                // The normal Media3 path above remains the fast path for all already-PASS formats.
                if(cancel.get()) throw new IOException("operation cancelled");
                LegacyVideoFallback legacy=new LegacyVideoFallback(a);
                boolean fallbackOk=legacy.transcode(local,out,original,sourceDuration);
                if(!fallbackOk){
                    String legacyError=legacy.getLastError();
                    throw new IOException(cleanError(legacyError!=null&&!legacyError.isEmpty()?legacyError:error));
                }
            }
