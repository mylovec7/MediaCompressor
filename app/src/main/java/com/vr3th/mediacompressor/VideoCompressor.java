...
public final class VideoCompressor {
    ...
    void run(Uri src, AtomicBoolean cancel){
        ...
        try{
            ...
            String inputExt=extensionFromUri(src);
            if(!isSupportedInput(inputExt)) throw new IOException("Format tidak termasuk daftar Video Compressor.");
            ...
            String error=transform(mediaItemFor(local),false,out,cancel);
            if(cancel.get()) throw new IOException("operation cancelled");
            if(error!=null || !out.exists() || out.length()<=0){
                safe(out);
                throw new IOException(cleanError(error));
            }
        ...
        try(android.database.Cursor c=a.getContentResolver().query(src,new String[]{android.provider.OpenableColumns.DISPLAY_NAME},null,null,null)){
            if(c!=null&&c.moveToFirst()){
                ...
                if(i>=0){
                    ...
                    if(n!=null){
                        int d=n.lastIndexOf('.');
                        if(d>=0&&d<n.length()-1)return n.substring(d).toLowerCase(Locale.US);
                    }
                }
            }
    ...
    private String mimeForExtension(String name){
        ...
        if(x.endsWith(".mp4")||x.endsWith(".m4v"))return "video/mp4";
        ...
        if(x.endsWith(".f4v"))return "video/x-flv";
        return null;
    }

    private boolean isSupportedInput(String name){
        String x=name.toLowerCase(Locale.US);
        return x.endsWith(".3g2")||x.endsWith(".3gp")||x.endsWith(".f4v")||
               x.endsWith(".m4v")||x.endsWith(".mkv")||x.endsWith(".mov")||
               x.endsWith(".mp4")||x.endsWith(".qt")||x.endsWith(".webm");
    }
