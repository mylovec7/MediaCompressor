...
class MainActivity : ComponentActivity() {
        ...
        runJob {
            when {
                ...
                mime.contains("zip") || item.name.endsWith(".zip", true) -> {
                    ...
                    finalizeResult(videoEngine.processVideo(item.uri, meta, plan, item.name, onStatus), MediaType.VIDEO, "video/mp4", true, onFinish)
                }
            }
        }
    }
    ...
    private suspend fun finalizeResult(res: ProcessResult, type: MediaType, mimeType: String, rejectIfLarger: Boolean, onFinish: (ProcessResult) -> Unit) {
        ...
        historyStore.saveItem(HistoryItem(res.originalName, res.originalSize, res.compressedSize, FormatUtils.getCurrentDateFormatted(), saved.toString(), type))
        try { File(res.outputPath).delete() } catch (_: Exception) {}
