...
class VideoEngine(private val context: Context) {
    ...
    fun remuxOrDirectCopy(
        ...
        try {
            ...
            muxer.start()
            val buffer = ByteBuffer.allocateDirect(max(4 * 1024 * 1024, maxInputBufferForTracks(extractor)))
    ...
    private fun compressNative(
        ...
        try {
            ...
            var encoderMime = chooseSafeVideoEncoder(plan.targetCodec)
            var codec: MediaCodec? = null
            var color = -1
            fun createAndConfigure(mime: String): MediaCodec {
                val c = MediaCodec.createEncoderByType(mime)
                val caps = c.codecInfo.getCapabilitiesForType(mime)
                val chosenColor = caps.colorFormats.firstOrNull { it == MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar }
                    ?: caps.colorFormats.firstOrNull { it == MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar }
                    ?: throw IllegalStateException("No compatible YUV420 encoder format available for $mime")
            ...
            try {
                codec = createAndConfigure(encoderMime)
            } catch (_: Exception) {
                if (encoderMime != MediaFormat.MIMETYPE_VIDEO_AVC) {
                    encoderMime = MediaFormat.MIMETYPE_VIDEO_AVC
                    codec = createAndConfigure(encoderMime)
                } else throw IllegalStateException("No usable AVC encoder for ${width}x${height}.")
            }
            encoder = codec
            codec.start()
            muxer = MediaMuxer(out.absolutePath, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4)
            val audioInfo = findMuxableAudioTrack(uri)
            val durationUs = info.durationMs.coerceAtLeast(1) * 1000L
            val intervalUs = 1_000_000L / fps
            val total = (((durationUs + intervalUs - 1L) / intervalUs).coerceAtMost(Int.MAX_VALUE.toLong())).toInt().coerceAtLeast(1)
            val bi = MediaCodec.BufferInfo()
            var frameIndex = 0
                ...
                while (true) {
                    when (val status = codec.dequeueOutputBuffer(bi, if (flush) 20_000 else 0)) {
                        ...
                        MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> {
                            ...
                            if (buffer != null && bi.size > 0 && muxerStarted && muxTrack >= 0) {
                                val start = bi.offset.coerceAtLeast(0)
                                val end = (start + bi.size).coerceAtMost(buffer.capacity())
                                if (end > start) {
                                    buffer.position(start); buffer.limit(end)
            ...
            while (frameIndex < total) {
                ...
                if (inputIndex >= 0) {
                    ...
                    input.clear()
                    val timeUs = (frameIndex * intervalUs).coerceAtMost((durationUs - 1L).coerceAtLeast(0L))
                    val frame = decodeFrameSafely(retriever, timeUs, durationUs)
                        ?: throw IllegalStateException("Decoder returned no frame near ${timeUs / 1000} ms")
            ...
            while (!eosQueued) {
                ...
                if (eosIndex >= 0) {
                    codec.queueInputBuffer(eosIndex, 0, 0, durationUs, MediaCodec.BUFFER_FLAG_END_OF_STREAM)
                    eosQueued = true
                } else {
                    drain()
                }
            ...
            return ProcessResult(true, originalName, info.size, out.length(),
                ...
                info.videoCodec ?: "Unknown", "${encoderMime} / Adaptive Native Encode + Audio Copy",
                System.currentTimeMillis() - start, out.absolutePath)
        } catch (e: Exception) {
            out.delete()
    ...
    private fun findMuxableAudioTrack(uri: Uri): Pair<Int, MediaFormat>? {
        val e = MediaExtractor()
        return try {
            e.setDataSource(context, uri, null)
            for (i in 0 until e.trackCount) {
                val f = e.getTrackFormat(i)
                // MP4 muxing is reliably supported for AAC. Do not blindly copy
                // Opus/AC3/etc. into an MP4 container and then report success.
                if (f.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_AUDIO_AAC) return i to f
            }
            null
        } finally { try { e.release() } catch (_: Exception) {} }
    }

    private fun chooseSafeVideoEncoder(requestedMime: String): String {
        return try {
            try {
                val c = MediaCodec.createEncoderByType(requestedMime)
                c.release()
                requestedMime
            } catch (_: Exception) {
    ...
    private fun decodeFrameSafely(retriever: MediaMetadataRetriever, timeUs: Long, durationUs: Long): Bitmap? {
        ...
        for (option in options) {
            try {
                val frame = retriever.getFrameAtTime(safe, option)
                if (frame != null) return frame
            } catch (_: RuntimeException) {
                // Try the next extraction strategy. Some vendor decoders reject
                // exact timestamps even though sync-frame extraction works.
            }
        }
        return null
    }
    ...
    private fun copyAudio(uri: Uri, trackIndex: Int, muxer: MediaMuxer, outTrack: Int) {
        ...
        try {
            ...
            val buffer = ByteBuffer.allocateDirect(max(512 * 1024, (if (format.containsKey(MediaFormat.KEY_MAX_INPUT_SIZE)) format.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE) else 512 * 1024)))
            val bi = MediaCodec.BufferInfo()
            while (true) {
                if (e.sampleTrackIndex < 0) break
                buffer.clear()
                val n = e.readSampleData(buffer, 0)
                if (n > 0) {
                    bi.offset = 0; bi.size = n; bi.presentationTimeUs = e.sampleTime; bi.flags = e.sampleFlags
                    muxer.writeSampleData(outTrack, buffer, bi)
                }
                e.advance()
            }
    ...
    private fun maxInputBufferForTracks(extractor: MediaExtractor): Int {
        var maxSize = 0
        for (i in 0 until extractor.trackCount) {
            val f = extractor.getTrackFormat(i)
            maxSize = max(maxSize, (if (f.containsKey(MediaFormat.KEY_MAX_INPUT_SIZE)) f.getInteger(MediaFormat.KEY_MAX_INPUT_SIZE) else 0))
        }
    ...
    private fun writeBitmapYuv420(bitmap: Bitmap, out: ByteBuffer, colorFormat: Int, width: Int, height: Int) {
        ...
        if (out.remaining() < yuv.size) throw IllegalStateException("Encoder input buffer is too small for ${width}x${height} YUV420 frame.")
        out.put(yuv)
    }
