...
class MediaToolEngine(private val context: Context) {
    ...
    fun reverseVideo(uri: Uri, originalName: String): ProcessResult {
        ...
        try {
            ...
            val fps = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_CAPTURE_FRAMERATE)?.toFloatOrNull()?.coerceIn(1f, 60f) ?: 30f
            ...
            val frameIntervalUs = (1_000_000L / fps).coerceAtLeast(1L)
            val totalFrames = ((durationMs * 1000L) / frameIntervalUs).toInt() + 1
            var fed = 0
            for (i in totalFrames - 1 downTo 0) {
                val timeUs = (i * frameIntervalUs).coerceAtMost(durationMs * 1000L)
                ...
                try {
                    ...
                    encoder.queueInputBuffer(inputIndex, 0, safeW * safeH * 3 / 2, fed * frameIntervalUs, 0)
                    fed++
            ...
            if (eosIndex >= 0) encoder.queueInputBuffer(eosIndex, 0, 0, fed * frameIntervalUs, MediaCodec.BUFFER_FLAG_END_OF_STREAM)
            var done = false
            while (!done) {
                ...
                when {
                    ...
                    status == MediaCodec.INFO_TRY_AGAIN_LATER -> done = fed == 0
                    status >= 0 -> { val b = encoder.getOutputBuffer(status); if (b != null && bufferInfo.size > 0 && muxTrack >= 0) { b.position(bufferInfo.offset); b.limit(bufferInfo.offset + bufferInfo.size); muxer.writeSampleData(muxTrack, b, bufferInfo) }; val eos = (bufferInfo.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0; encoder.releaseOutputBuffer(status, false); if (eos) done = true }
                }
            }
    ...
    fun mergeVideos(uris: List<Uri>, originalName: String): ProcessResult {
        ...
        return try {
            ...
            val buffer = ByteBuffer.allocateDirect(1024 * 1024); val bi = MediaCodec.BufferInfo()
            for ((index, uri) in uris.withIndex()) {
                ...
                for (j in current.indices) {
                    val f = e.getTrackFormat(current[j]); require((f.getString(MediaFormat.KEY_MIME) ?: "") == (baseTracks[j].getString(MediaFormat.KEY_MIME) ?: "")) { "Video/audio codecs differ; merge requires compatible streams." }
                ...
                for (j in current.indices) offsets[j] += lastTimes[j]
            }
    ...
    private fun copyTrack(e: MediaExtractor, muxer: MediaMuxer, outTrack: Int) { val buffer=ByteBuffer.allocateDirect(1024*1024);val bi=MediaCodec.BufferInfo();while(true){val ti=e.sampleTrackIndex;if(ti<0)break;buffer.clear();val n=e.readSampleData(buffer,0);if(n>0){bi.offset=0;bi.size=n;bi.presentationTimeUs=e.sampleTime;bi.flags=e.sampleFlags;muxer.writeSampleData(outTrack,buffer,bi)};e.advance()} }
