...
class SmartEngine(private val context: Context) {
    fun analyzeVideo(uri: Uri): MediaMetadataInfo {
        ...
        // AVC is the safest cross-device output. Keep HEVC only when the source is already HEVC.
        val preferred = when {
            videoCodec.equals(MediaFormat.MIMETYPE_VIDEO_HEVC, true) && hevc -> MediaFormat.MIMETYPE_VIDEO_HEVC
    ...
    fun makeVideoPlan(info: MediaMetadataInfo): CompressionPlan {
        ...
        val sourceVideo = info.videoCodec?.lowercase() ?: ""
        val sourceAudio = info.audioCodec?.lowercase() ?: ""
        val streamCopySafe = (sourceVideo == MediaFormat.MIMETYPE_VIDEO_AVC.lowercase() || sourceVideo == MediaFormat.MIMETYPE_VIDEO_HEVC.lowercase()) &&
            (sourceAudio.isEmpty() || sourceAudio == MediaFormat.MIMETYPE_AUDIO_AAC.lowercase())
        if (streamCopySafe && info.bitrate in 1..500_000 && width <= 1280 && height <= 1280) {
            return CompressionPlan(true, info.videoCodec ?: MediaFormat.MIMETYPE_VIDEO_AVC, width and -2, height and -2,
                ...
                "Already efficient — using safe stream copy to preserve quality.")
        }
        val scale = when {
        ...
        var targetW = ((width * scale).toInt() and -2).coerceAtLeast(2)
        var targetH = ((height * scale).toInt() and -2).coerceAtLeast(2)
        // Keep the frame working-set small even for 4K/8K inputs. A 1080p ceiling
        // is a deliberate battery/RAM trade-off for an ultra-light native engine.
        val maxPixels = 1920L * 1080L
        if (targetW.toLong() * targetH > maxPixels) {
            val ratio = kotlin.math.sqrt(maxPixels.toDouble() / (targetW.toLong() * targetH).toDouble())
            targetW = ((targetW * ratio).toInt() and -2).coerceAtLeast(2)
            targetH = ((targetH * ratio).toInt() and -2).coerceAtLeast(2)
        }
        ...
        val sourceCap = if (info.bitrate > 0) (info.bitrate * 0.82).toLong() else Long.MAX_VALUE
        val targetBitrate = minOf(calculated, sourceCap).coerceIn(300_000L, if (targetW >= 1920) 4_000_000L else 2_500_000L).toInt()
