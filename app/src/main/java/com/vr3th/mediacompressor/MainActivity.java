...
public class MainActivity extends Activity{
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(Color.rgb(5,10,8));getWindow().setNavigationBarColor(Color.rgb(5,10,8));ops=new MediaOps(this); EngineModules.initPdf(this); splash();}
    ...
    void ui(){ScrollView sc=new ScrollView(this);sc.setFillViewport(true);sc.setBackgroundColor(0xff050a08);root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(14),dp(14),dp(14),dp(24));sc.addView(root);setContentView(sc);
        ...
        add("◈ VIDEO COMPRESS","SMART ENCODE • SIZE GUARD","Compress video; larger/equal output is never accepted.",()->pick("video/*"));
        ...
        add("▣ VIDEO MERGE","LOSSLESS REMUX","Merge codec-compatible videos without re-encoding.",()->{mode=19;pickMulti("video/*");});
        add("▣ VIDEO TRIM","LOSSLESS REMUX","Cut compatible encoded tracks without re-encoding.",()->{mode=20;pick("video/*");});
        add("▣ MUTE VIDEO","AUDIO STRIP","Remove the audio track without re-encoding video.",()->{mode=21;pick("video/*");});
        add("▣ EXTRACT FRAME","VIDEO → PHOTO","Export a frame to Gallery.",()->{mode=22;pick("video/*");});
        add("♫ AUDIO TRIM","LOSSLESS REMUX","Trim supported encoded audio into M4A container.",()->{mode=30;pick("audio/*");});
        ...
        TextView more=tv("// CODEC NOTE\nMP3/GIF encoders and true PDF password encryption are intentionally not faked. Those require heavier codec/PDF engines and would conflict with the Ultra Lite target.",9,0xff7faf98);more.setPadding(dp(4),dp(8),dp(4),dp(12));root.addView(more);
        ...
        new AlertDialog.Builder(this).setTitle("SPLIT PDF").setView(box)
            .setNegativeButton("CANCEL",null).setPositiveButton("RUN",(d,w)->{
                try{
                    ...
                    if(a1<1||b1<a1) throw new IllegalArgumentException();
                    ops.splitPdf(u,a1-1,b1-1,res());
    ...
    void timeDialog(Uri u,boolean audio,boolean mute){final EditText from=new EditText(this);from.setHint("start seconds");final EditText to=new EditText(this);to.setHint("end seconds");LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(18),dp(4),dp(18),dp(4));box.addView(from);box.addView(to);new AlertDialog.Builder(this).setTitle(audio?"AUDIO TRIM":"VIDEO TRIM").setView(box).setNegativeButton("CANCEL",null).setPositiveButton("RUN",(d,w)->{try{long a=(long)(Double.parseDouble(from.getText().toString())*1000000);long b=(long)(Double.parseDouble(to.getText().toString())*1000000);if(audio)ops.audioTrim(u,a,b,res());else ops.trimOrMuteVideo(u,a,b,mute,res());}catch(Exception e){showStatus("> ORIGINAL PRESERVED\nInvalid time range.");}}).show();}
    void compressPhoto(Uri u){new Thread(()->{File f=null;Bitmap b=null;try{long s=sourceSize(u);b=decodeOrientedBitmap(u);if(b==null)throw new IOException();f=tmp(".jpg");try(FileOutputStream o=new FileOutputStream(f)){b.compress(Bitmap.CompressFormat.JPEG,86,o);}long d=f.length();if(d<=0||(s>0&&d>=s)){safe(f);showStatus("> OPTIMAL RESULT\nNo smaller output was found.\nOriginal quality preserved ✓");return;}publishImage(f,false);safe(f);showStatus("> COMPRESSION COMPLETE\n"+fmtSize(s)+" → "+fmtSize(d)+"\n✓ OUTPUT VERIFIED\n✓ ORIGINAL PRESERVED\n✓ SAVED TO GALLERY");}catch(Exception e){safe(f);showStatus("> ORIGINAL PRESERVED\nA better version could not be created.");}finally{if(b!=null)b.recycle();}}).start();}
    Bitmap decodeOrientedBitmap(Uri u)throws IOException{
        ...
        int o=0;
        try{
            androidx.exifinterface.media.ExifInterface ex=new androidx.exifinterface.media.ExifInterface(getContentResolver().openInputStream(u));
            int v=ex.getAttributeInt(androidx.exifinterface.media.ExifInterface.TAG_ORIENTATION,androidx.exifinterface.media.ExifInterface.ORIENTATION_NORMAL);
            if(v==androidx.exifinterface.media.ExifInterface.ORIENTATION_ROTATE_90)o=90;
            else if(v==androidx.exifinterface.media.ExifInterface.ORIENTATION_ROTATE_180)o=180;
            else if(v==androidx.exifinterface.media.ExifInterface.ORIENTATION_ROTATE_270)o=270;
        }catch(Exception ignored){}
        if(o==0)return b;
        Matrix m=new Matrix();m.postRotate(o);
    ...
    Uri publishImage(File f,boolean webp)throws IOException{
        String n="IMG_"+System.currentTimeMillis()+"."+(webp?"webp":"jpg");
        return publishGeneric(f,webp?"image/webp":"image/jpeg","MediaCompressor/Images",n);
    }
    Uri publishVideo(File f)throws IOException{
        return publishGeneric(f,"video/mp4","MediaCompressor/Videos","VID_"+System.currentTimeMillis()+".mp4");
    }
    Uri publishAudio(File f)throws IOException{
        return publishGeneric(f,"audio/mp4","MediaCompressor/Audio","AUD_"+System.currentTimeMillis()+".m4a");
    }
    Uri publishPdf(File f)throws IOException{
        return publishGeneric(f,"application/pdf","MediaCompressor/Documents","MC_"+System.currentTimeMillis()+".pdf");
    }
    Uri publishZip(File f)throws IOException{
        return publishGeneric(f,"application/zip","MediaCompressor/Archives","MC_"+System.currentTimeMillis()+".zip");
    }
    Uri publishGeneric(File f,String mime,String rel,String name)throws IOException{
        ContentValues v=new ContentValues();
        v.put(MediaStore.MediaColumns.DISPLAY_NAME,name);
        v.put(MediaStore.MediaColumns.MIME_TYPE,mime);
        if(Build.VERSION.SDK_INT>=29)v.put(MediaStore.MediaColumns.RELATIVE_PATH,rel.endsWith("/")?rel:rel+"/");
        Uri u=getContentResolver().insert(MediaStore.Files.getContentUri("external"),v);
        if(u==null)throw new IOException("storage");
        try(OutputStream o=getContentResolver().openOutputStream(u);InputStream in=new FileInputStream(f)){copy(in,o);}
        return u;
    }
    void publishDirectory(File d)throws IOException{
        if(d==null||!d.isDirectory())throw new IOException("directory");
        // Extracted ZIPs are copied into the public MediaCompressor/Extract tree.
        // The private working directory is removed only after every file is exported.
        ...
        for(File f:files){
            ...
            if(Build.VERSION.SDK_INT>=29)v.put(MediaStore.MediaColumns.RELATIVE_PATH,"MediaCompressor/Extract/"+(dir.isEmpty()?"":dir+"/"));
            Uri out=getContentResolver().insert(MediaStore.Files.getContentUri("external"),v);
            if(out==null)throw new IOException("storage");
            try(OutputStream o=getContentResolver().openOutputStream(out);InputStream in=new FileInputStream(f)){copy(in,o);}
        }
        deleteTree(d);
    }
    ...
    File copyToTemp(Uri u,String ext)throws IOException{File f=tmp(ext);try(InputStream in=getContentResolver().openInputStream(u);OutputStream o=new FileOutputStream(f)){copy(in,o);}return f;}
    ...
    void repackZip(Uri u){new Thread(()->{File f=tmp(".zip");try(ZipInputStream in=new ZipInputStream(getContentResolver().openInputStream(u));ZipOutputStream out=new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(f)))){ZipEntry e;byte[] b=new byte[64*1024];while((e=in.getNextEntry())!=null){ZipEntry n=new ZipEntry(e.getName());out.putNextEntry(n);int z;while((z=in.read(b))!=-1)out.write(b,0,z);out.closeEntry();in.closeEntry();}long s=sourceSize(u),d=f.length();if(d>=s){safe(f);showStatus("> OPTIMAL RESULT\nOriginal ZIP preserved ✓");return;}publishZip(f);safe(f);showStatus("> ZIP OPTIMIZED\n"+fmtSize(s)+" → "+fmtSize(d)+"\n✓ ORIGINAL PRESERVED");}catch(Exception e){safe(f);showStatus("> ORIGINAL PRESERVED\nZIP optimization could not be completed.");}}).start();}
}
