...
final class MediaOps {
    ...
    void mergeVideos(ArrayList<Uri> list,Result r){new Thread(()->{File out=tmp(".mp4");MediaMuxer mux=null;try{if(list.size()<2)throw new IOException("need two videos");MediaExtractor first=new MediaExtractor();first.setDataSource(a,list.get(0),null);int v=-1,au=-1;for(int i=0;i<first.getTrackCount();i++){String m=first.getTrackFormat(i).getString(MediaFormat.KEY_MIME);if(m!=null&&m.startsWith("video/"))v=i;else if(m!=null&&m.startsWith("audio/"))au=i;}if(v<0)throw new IOException("video track");mux=new MediaMuxer(out.getAbsolutePath(),MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);int mv=mux.addTrack(first.getTrackFormat(v));int ma=au>=0?mux.addTrack(first.getTrackFormat(au)):-1;mux.start();long offset=0;ByteBuffer buf=ByteBuffer.allocateDirect(512*1024);MediaCodec.BufferInfo bi=new MediaCodec.BufferInfo();for(Uri u:list){
                    long segmentOffset=offset;
                    MediaExtractor ex=new MediaExtractor();ex.setDataSource(a,u,null);
                    int vt=-1,at=-1;for(int i=0;i<ex.getTrackCount();i++){String m=ex.getTrackFormat(i).getString(MediaFormat.KEY_MIME);if(m!=null&&m.startsWith("video/"))vt=i;else if(m!=null&&m.startsWith("audio/"))at=i;}
                    if(vt<0)throw new IOException("video track");
                    MediaFormat vf=ex.getTrackFormat(vt);
                    if(!vf.getString(MediaFormat.KEY_MIME).equals(first.getTrackFormat(v).getString(MediaFormat.KEY_MIME))||vf.getInteger(MediaFormat.KEY_WIDTH)!=first.getTrackFormat(v).getInteger(MediaFormat.KEY_WIDTH)||vf.getInteger(MediaFormat.KEY_HEIGHT)!=first.getTrackFormat(v).getInteger(MediaFormat.KEY_HEIGHT))throw new IOException("Video tracks are not codec-compatible.");
                    ex.selectTrack(vt);long videoDur=0;
                    while(true){int n=ex.readSampleData(buf,0);if(n<0)break;long pts=ex.getSampleTime();if(pts>=0)videoDur=Math.max(videoDur,pts);bi.offset=0;bi.size=n;bi.presentationTimeUs=Math.max(0,segmentOffset+pts);bi.flags=ex.getSampleFlags();mux.writeSampleData(mv,buf,bi);ex.advance();}
                    if(at>=0&&ma>=0){
                        MediaFormat af=ex.getTrackFormat(at);
                        if(!af.getString(MediaFormat.KEY_MIME).equals(first.getTrackFormat(au).getString(MediaFormat.KEY_MIME)))throw new IOException("Audio tracks are not codec-compatible.");
                        ex.selectTrack(at);
                        while(true){int n=ex.readSampleData(buf,0);if(n<0)break;long pts=ex.getSampleTime();bi.offset=0;bi.size=n;bi.presentationTimeUs=Math.max(0,segmentOffset+pts);bi.flags=ex.getSampleFlags();mux.writeSampleData(ma,buf,bi);ex.advance();}
                        ex.unselectTrack(at);
                    }
                    ex.unselectTrack(vt);
                    long dur=vf.containsKey(MediaFormat.KEY_DURATION)?vf.getLong(MediaFormat.KEY_DURATION):(videoDur+1);
                    offset+=Math.max(0,dur);
                    ex.release();
                }first.release();mux.stop();mux.release();mux=null;long s=0;for(Uri u:list){long z=a.sourceSize(u);if(z>0)s+=z;}long d=out.length();if(d<=0||(s>0&&d>=s)){safe(out);done(r,"> OPTIMAL RESULT\\nCombined output was not smaller.\\nOriginals preserved ✓");return;}a.publishVideo(out);safe(out);done(r,"> VIDEO MERGE COMPLETE\\n"+a.fmtSize(s)+" → "+a.fmtSize(d)+"\\n✓ VERIFIED\\n✓ ORIGINALS PRESERVED");}catch(Exception e){if(mux!=null)try{mux.release();}catch(Exception ignored){}safe(out);fail(r,"> ORIGINALS PRESERVED\\nVideos are not codec-compatible for lossless merge.");}}).start();}
    ...
    void zipCreate(ArrayList<Uri> list,Result r){
        new Thread(()->{
            ...
            try(ZipOutputStream z=new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(f)))){
                ...
                for(Uri u:list){
                    if(a.cancelled.get())throw new IOException("cancelled");
                    String name="file_"+(++i); try(Cursor c=a.getContentResolver().query(u,new String[]{android.provider.OpenableColumns.DISPLAY_NAME},null,null,null)){if(c!=null&&c.moveToFirst()){String n=c.getString(0);if(n!=null&&!n.isEmpty())name=n;}}
    ...
    void zipExtract(Uri u,Result r){new Thread(()->{File local=null;try{local=a.copyToTemp(u,".zip");String baseName="Extracted";try(Cursor c=a.getContentResolver().query(u,new String[]{android.provider.OpenableColumns.DISPLAY_NAME},null,null,null)){if(c!=null&&c.moveToFirst()){String n=c.getString(0);if(n!=null&&!n.isEmpty()){int dot=n.lastIndexOf('.');baseName=dot>0?n.substring(0,dot):n;}}}baseName=baseName.replaceAll("[\\\\/:*?\"<>|]","_");String relRoot="MediaCompressor/Extract/"+baseName;try(ZipFile z=new ZipFile(local)){Enumeration<? extends ZipEntry> en=z.entries();byte[] b=new byte[64*1024];while(en.hasMoreElements()){if(a.cancelled.get())throw new IOException("cancelled");ZipEntry e=en.nextElement();String name=e.getName().replace('\\\\','/');if(e.isDirectory())continue;if(name.startsWith("/")||name.contains("../")||name.equals("..")||name.contains("/../"))throw new IOException("unsafe ZIP entry");String safeName=name.replaceAll("^/+","").replaceAll("[\\\\:*?\"<>|]","_");int slash=safeName.lastIndexOf('/');String dir=slash>=0?safeName.substring(0,slash):"";String fileName=slash>=0?safeName.substring(slash+1):safeName;String rel=relRoot+(dir.isEmpty()?"":"/"+dir);ContentValues v=new ContentValues();v.put(MediaStore.MediaColumns.DISPLAY_NAME,fileName);v.put(MediaStore.MediaColumns.MIME_TYPE,"application/octet-stream");if(Build.VERSION.SDK_INT>=29)v.put(MediaStore.MediaColumns.RELATIVE_PATH,rel+"/");Uri out=a.getContentResolver().insert(MediaStore.Files.getContentUri("external"),v);if(out==null)throw new IOException("storage");try(InputStream in=z.getInputStream(e);OutputStream o=a.getContentResolver().openOutputStream(out)){int n;while((n=in.read(b))!=-1){if(a.cancelled.get()){try{a.getContentResolver().delete(out,null,null);}catch(Exception ignored){}throw new IOException("cancelled");}o.write(b,0,n);}}}}}done(r,"> ZIP EXTRACT COMPLETE\\nMediaCompressor/Extract/"+baseName+"/\\n✓ OUTPUT EXPORTED");}catch(Exception e){fail(r,a.cancelled.get()?"> ORIGINAL PRESERVED\\nExtraction cancelled safely.":"> ORIGINAL PRESERVED\\nZIP could not be extracted.");}finally{if(local!=null)local.delete();}}).start();}\n    private boolean validImage(File f){
