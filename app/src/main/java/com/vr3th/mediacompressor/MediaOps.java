...
final class MediaOps {
    ...
    void trimOrMuteVideo(Uri u,long fromUs,long toUs,boolean mute,Result r){
        ...
        new Thread(()->{
            ...
            try{
                ...
                runTransformerSingle(edited,out,r,"VIDEO OPERATION COMPLETE");
            }catch(Exception e){
                safe(out);
    ...
    private void runTransformerSingle(EditedMediaItem edited,File out,Result r,String label)throws Exception{
        ...
        long s=sizeFromEdited(edited); long d=out.length();
        a.publishVideo(out); safe(out);
        done(r,"> "+label+"\n"+a.fmtSize(s)+" → "+a.fmtSize(d)+"\n✓ VERIFIED\n✓ ORIGINAL PRESERVED");
    }

    private long sizeFromEdited(EditedMediaItem edited){
        try{return size(edited.mediaItem.localConfiguration.uri);}catch(Exception e){return -1;}
    }
    ...
    void mergeVideos(ArrayList<Uri> list,Result r){
        new Thread(()->{
            ...
            try{
                ...
                // One fixed output canvas, derived from the first source's DISPLAY geometry.
                // Every input is fitted into that canvas with letterboxing/pillarboxing.
                // This is deliberately not stretch-to-fit: mixed portrait/landscape inputs
                // must never become squeezed, flattened or geometrically distorted.
                int[] target=displayVideoSize(list.get(0));
                int targetW=target[0], targetH=target[1];
                if(targetW<=0||targetH<=0)throw new IOException("video geometry unavailable");
                targetW=Math.max(2,targetW&~1); targetH=Math.max(2,targetH&~1);
                ...
                for(Uri u:list){
                    ...
                    try{
                        probe=new MediaExtractor(); probe.setDataSource(a,u,null);
                        boolean video=false;
                        for(int i=0;i<probe.getTrackCount();i++){
                            String m=probe.getTrackFormat(i).getString(MediaFormat.KEY_MIME);
                            if(m!=null&&m.startsWith("video/")){video=true;break;}
                        }
                        if(!video)throw new IOException("video track missing");
                    }finally{if(probe!=null)try{probe.release();}catch(Exception ignored){}}

                    MediaItem mi=MediaItem.fromUri(u);
                    ChannelMixingAudioProcessor channelMix=new ChannelMixingAudioProcessor();
                    for(int channels=1;channels<=6;channels++){
                        try{channelMix.putChannelMixingMatrix(ChannelMixingMatrix.createForConstantPower(channels,2));}catch(Exception ignored){}
                    }
                    Effects effects=new Effects(
                            Arrays.asList(new ToInt16PcmAudioProcessor(),channelMix),
                ...
                h.post(()->{
                    try{
                        ...
                        t.addListener(new Transformer.Listener(){
                            ...
                            @Override public void onError(Composition c,ExportResult result,ExportException e){
                                if(finished[0])return;finished[0]=true;
                                error[0]=e.getMessage()==null?"merge export failed":e.getMessage();latch.countDown();
                            }
                        });
                        t.start(composition,out.getAbsolutePath());
                    }catch(Exception e){
                        if(finished[0])return;finished[0]=true;
                        error[0]=e.getMessage()==null?"merge could not start":e.getMessage();latch.countDown();
                    }
                });
                while(!latch.await(200,java.util.concurrent.TimeUnit.MILLISECONDS)){
                ...
                if(error[0]!=null||!validVideo(out)||out.length()<=0){
                    safe(out);fail(r,"> ORIGINALS PRESERVED\nVideo merge failed safely.\n"+(error[0]==null?"Output validation failed.":error[0]));return;
                }
                long d=out.length();
                a.publishVideo(out); safe(out);
                done(r,"> VIDEO MERGE COMPLETE\n"+a.fmtSize(d)+" output\n✓ ASPECT RATIO PRESERVED\n✓ CODECS NORMALIZED\n✓ VIDEO + AUDIO CONTINUOUS\n✓ VERIFIED\n✓ ORIGINALS PRESERVED");
            }catch(Exception e){
                safe(out);fail(r,a.cancelled.get()?"> ORIGINALS PRESERVED\nMerge cancelled safely.":"> ORIGINALS PRESERVED\nVideo merge failed safely.\n"+(e.getMessage()==null?"Unsupported media.":e.getMessage()));
            }
        }).start();
    }
    ...
    void audioTrim(Uri u,long fromUs,long toUs,Result r){new Thread(()->{
        ...
        try{
            ...
            MediaItem item=new MediaItem.Builder()
                    .setUri(u)
                    .setClippingConfiguration(new MediaItem.ClippingConfiguration.Builder()
                            .setStartPositionMs(fromUs/1000L)
                            .setEndPositionMs(toUs/1000L)
                            .build())
                    .build();
            ...
            h.post(()->{
                try{
                    ...
                    t.addListener(new Transformer.Listener(){
                        ...
                        @Override public void onError(Composition c,ExportResult result,ExportException e){
                            if(finished[0])return;finished[0]=true;
                            error[0]=e.getMessage()==null?"audio transformation failed":e.getMessage();latch.countDown();
                        }
                    });
                    t.start(edited,out.getAbsolutePath());
                }catch(Exception e){
                    if(finished[0])return;finished[0]=true;error[0]=e.getMessage()==null?"audio transformation failed":e.getMessage();latch.countDown();
                }
            });
            while(!latch.await(200,java.util.concurrent.TimeUnit.MILLISECONDS)){
                if(a.cancelled.get())try{if(box[0]!=null)box[0].cancel();}catch(Exception ignored){}
            ...
            if(error[0]!=null||!out.exists()||out.length()<=0){safe(out);throw new IOException(error[0]==null?"audio output invalid":error[0]);}
            long original=size(u),d=out.length();
            a.publishAudioFormat(out,"m4a");safe(out);
