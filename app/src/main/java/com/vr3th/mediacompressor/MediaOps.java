...
final class MediaOps {
    ...
    private void runTransformerSingle(EditedMediaItem edited,File out,Result r,String label,long sourceBytes)throws Exception{
        ...
        h.post(()->{
            try{
                Transformer t=VideoEngine.builder(a.getApplicationContext(),VideoEngine.bestVideoMime(),MimeTypes.AUDIO_AAC).build();
                box[0]=t;
        ...
        new Thread(()->{
            ...
            try{
                ...
                for(Uri u:list){
                    ...
                    File local=a.copyToTemp(u,".mp4"); locals.add(local);
    ...
    void videoToMp3(Uri u,Result r){new Thread(()->{
        File local=null,out=null;
        try{
            local=a.copyToTemp(u,".media");
            MediaExtractor ex=new MediaExtractor();ex.setDataSource(local.getAbsolutePath());
            ...
            if(track<0)throw new IOException("Video tidak memiliki audio.");
            String mime=fmt.getString(MediaFormat.KEY_MIME);
            out=tmp(".mp3");
            // Zero-decode fast path when the source already contains MP3 audio.
            if("audio/mpeg".equalsIgnoreCase(mime)){
                ex.selectTrack(track);byte[] b=new byte[128*1024];java.nio.ByteBuffer bb=java.nio.ByteBuffer.allocateDirect(256*1024);
                try(FileOutputStream o=new FileOutputStream(out)){
                    while(true){bb.clear();int n=ex.readSampleData(bb,0);if(n<0)break;bb.flip();byte[] x=new byte[n];bb.get(x);o.write(x);ex.advance();if(a.cancelled.get())throw new IOException("cancelled");}
                }finally{ex.release();}
            }else{
                // Keep the app Ultra-Lite: use the device's MP3 encoder when one exists.
                if(!hasMp3Encoder())throw new IOException("MP3 encoder is not available on this device. No heavy codec pack is bundled.");
                transcodeAudioToMp3(local,track,fmt,out);
                ex.release();
            }
            if(!out.exists()||out.length()<=0)throw new IOException("MP3 output is invalid.");
            a.publishAudioFormat(out,"mp3");long d=out.length();safe(out);
            done(r,"> VIDEO → MP3 COMPLETE\n"+a.fmtSize(d)+" output\n✓ MP3 VERIFIED\n✓ ORIGINAL PRESERVED");
        }catch(Exception e){safe(out);fail(r,"> ORIGINAL PRESERVED\nVIDEO → MP3 could not be completed.\n"+(e.getMessage()==null?"MP3 encoder unavailable.":e.getMessage()));}
        finally{safe(local);}
    }).start();}

    private boolean hasMp3Encoder(){
        MediaCodecList l=new MediaCodecList(MediaCodecList.REGULAR_CODECS);
        for(MediaCodecInfo i:l.getCodecInfos())if(i.isEncoder())for(String t:i.getSupportedTypes())if("audio/mpeg".equalsIgnoreCase(t))return true;
        return false;
    }

    private void transcodeAudioToMp3(File local,int track,MediaFormat src,File out)throws Exception{
        String inMime=src.getString(MediaFormat.KEY_MIME);MediaCodec dec=MediaCodec.createDecoderByType(inMime);MediaCodec enc=MediaCodec.createEncoderByType("audio/mpeg");
        int rate=src.containsKey(MediaFormat.KEY_SAMPLE_RATE)?src.getInteger(MediaFormat.KEY_SAMPLE_RATE):44100;
        int ch=src.containsKey(MediaFormat.KEY_CHANNEL_COUNT)?Math.min(2,src.getInteger(MediaFormat.KEY_CHANNEL_COUNT)):2;
        android.os.Bundle eb=new android.os.Bundle();
        MediaFormat ef=MediaFormat.createAudioFormat("audio/mpeg",rate,ch);ef.setInteger(MediaFormat.KEY_BIT_RATE,128000);
        dec.configure(src,null,null,0);enc.configure(ef,null,null,MediaCodec.CONFIGURE_FLAG_ENCODE);dec.start();enc.start();
        MediaExtractor ex=new MediaExtractor();ex.setDataSource(local.getAbsolutePath());ex.selectTrack(track);
        boolean inEos=false,decEos=false,encEos=false;long timeout=10000;
        try(FileOutputStream o=new FileOutputStream(out)){
            while(!encEos){
                if(!inEos){int ii=dec.dequeueInputBuffer(timeout);if(ii>=0){java.nio.ByteBuffer ib=dec.getInputBuffer(ii);int n=ex.readSampleData(ib,0);if(n<0){dec.queueInputBuffer(ii,0,0,0,MediaCodec.BUFFER_FLAG_END_OF_STREAM);inEos=true;}else{long pts=ex.getSampleTime();dec.queueInputBuffer(ii,0,n,Math.max(0,pts),0);ex.advance();}}}
                if(!decEos){MediaCodec.BufferInfo di=new MediaCodec.BufferInfo();int oi=dec.dequeueOutputBuffer(di,timeout);if(oi>=0){java.nio.ByteBuffer db=dec.getOutputBuffer(oi);if(db!=null&&di.size>0){int ei=enc.dequeueInputBuffer(timeout);if(ei>=0){java.nio.ByteBuffer ebuf=enc.getInputBuffer(ei);ebuf.clear();db.position(di.offset);db.limit(di.offset+di.size);ebuf.put(db);enc.queueInputBuffer(ei,0,di.size,di.presentationTimeUs,(di.flags&MediaCodec.BUFFER_FLAG_END_OF_STREAM)!=0?MediaCodec.BUFFER_FLAG_END_OF_STREAM:0);}}if((di.flags&MediaCodec.BUFFER_FLAG_END_OF_STREAM)!=0)decEos=true;dec.releaseOutputBuffer(oi,false);} }
                if(decEos&&!encEos){int ei=enc.dequeueOutputBuffer(new MediaCodec.BufferInfo(),timeout);}
                MediaCodec.BufferInfo ai=new MediaCodec.BufferInfo();int ao=enc.dequeueOutputBuffer(ai,timeout);if(ao==MediaCodec.INFO_OUTPUT_FORMAT_CHANGED){}else if(ao>=0){java.nio.ByteBuffer ab=enc.getOutputBuffer(ao);if(ab!=null&&ai.size>0){ab.position(ai.offset);ab.limit(ai.offset+ai.size);byte[] data=new byte[ai.size];ab.get(data);o.write(data);}if((ai.flags&MediaCodec.BUFFER_FLAG_END_OF_STREAM)!=0)encEos=true;enc.releaseOutputBuffer(ao,false);}
                if(a.cancelled.get())throw new IOException("cancelled");
            }
        }finally{ex.release();try{dec.stop();}catch(Exception ignored){}try{enc.stop();}catch(Exception ignored){}dec.release();enc.release();}
    }
