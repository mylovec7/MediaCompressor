...
final class MediaOps {
    ...
    void trimOrMuteVideo(Uri u,long fromUs,long toUs,boolean mute,Result r){new Thread(()->{
        ...
        try{
            ...
            video=tmp(".video.mp4");NativeVideoEngine.encodeFrames(a,local,video,start,end,reverse,Math.max(500_000,estimateBitrate(local)),a.cancelled);
    ...
    void mergeVideos(ArrayList<Uri> list,Result r){new Thread(()->{
        File out=tmp(".mp4");ArrayList<File> locals=new ArrayList<>();
        try{if(list==null||list.size()<2)throw new IOException("select at least two videos");
            // Lightweight native merge: all inputs must expose compatible video/audio track formats.
            MediaExtractor first=new MediaExtractor();File base=a.copyToTemp(list.get(0),".media");locals.add(base);first.setDataSource(base.getAbsolutePath());MediaFormat vf=null,af=null;for(int i=0;i<first.getTrackCount();i++){MediaFormat f=first.getTrackFormat(i);String m=f.getString(MediaFormat.KEY_MIME);if(m!=null&&m.startsWith("video/"))vf=f;else if(m!=null&&m.startsWith("audio/"))af=f;}first.release();if(vf==null)throw new IOException("video track unavailable");
            // For safety and size, use the first file as the format template and append compatible samples.
            MediaMuxer mux=new MediaMuxer(out.getAbsolutePath(),MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);int vo=mux.addTrack(vf);int ao=af==null?-1:mux.addTrack(af);mux.start();long basePts=0;
            for(Uri u:list){File f=a.copyToTemp(u,".media");locals.add(f);MediaExtractor ex=new MediaExtractor();ex.setDataSource(f.getAbsolutePath());int vi=-1,ai=-1;for(int i=0;i<ex.getTrackCount();i++){String m=ex.getTrackFormat(i).getString(MediaFormat.KEY_MIME);if(m!=null&&m.startsWith("video/"))vi=i;else if(m!=null&&m.startsWith("audio/"))ai=i;}if(vi<0)throw new IOException("video track unavailable");long firstVideo=-1;ex.selectTrack(vi);java.nio.ByteBuffer b=java.nio.ByteBuffer.allocateDirect(2*1024*1024);while(true){b.clear();int n=ex.readSampleData(b,0);if(n<0)break;long pts=ex.getSampleTime();if(firstVideo<0)firstVideo=Math.max(0,pts);MediaCodec.BufferInfo bi=new MediaCodec.BufferInfo();bi.offset=0;bi.size=n;bi.presentationTimeUs=basePts+Math.max(0,pts-firstVideo);bi.flags=ex.getSampleFlags();mux.writeSampleData(vo,b,bi);ex.advance();}ex.unselectTrack(vi);if(ao>=0&&ai>=0){ex.selectTrack(ai);long firstAudio=-1;while(true){b.clear();int n=ex.readSampleData(b,0);if(n<0)break;long pts=ex.getSampleTime();if(firstAudio<0)firstAudio=Math.max(0,pts);MediaCodec.BufferInfo bi=new MediaCodec.BufferInfo();bi.offset=0;bi.size=n;bi.presentationTimeUs=basePts+Math.max(0,pts-firstAudio);bi.flags=ex.getSampleFlags();mux.writeSampleData(ao,b,bi);ex.advance();}ex.unselectTrack(ai);}basePts=Math.max(basePts,sourceDurationUs(Uri.fromFile(f)));ex.release();}
            mux.stop();mux.release();if(!validVideo(out))throw new IOException("merged output invalid");long d=out.length();a.publishVideo(out);safe(out);done(r,"> NATIVE VIDEO MERGE COMPLETE\n"+a.fmtSize(d)+" output\n✓ MEDIACODEC/MUXER\n✓ OUTPUT VERIFIED\n✓ ORIGINALS PRESERVED");
        }catch(Exception e){safe(out);fail(r,"> ORIGINALS PRESERVED\nVIDEO MERGE FAILED\n"+(e.getMessage()==null?"Inputs are not natively compatible.":e.getMessage()));}finally{for(File f:locals)safe(f);}
    }).start();}
