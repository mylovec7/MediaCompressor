    ...
    void zipCreate(ArrayList<Uri> list,Result r){
        new Thread(()->{
            ...
            try(ZipOutputStream z=new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(f)))){
                ...
                for(Uri u:list){
                    if(a.cancelled.get())throw new IOException("cancelled");
                    String name="file_"+(++i); String mime=a.getContentResolver().getType(u);try(Cursor c=a.getContentResolver().query(u,new String[]{android.provider.OpenableColumns.DISPLAY_NAME},null,null,null)){if(c!=null&&c.moveToFirst()){String n=c.getString(0);if(n!=null&&!n.isEmpty())name=n;}}
        ...
        new Thread(()->{
            ...
            try(ZipInputStream z=new ZipInputStream(new BufferedInputStream(a.getContentResolver().openInputStream(u)))){
                ...
                while((e=z.getNextEntry())!=null){
                    ...
                    try(FileOutputStream o=new FileOutputStream(out)){while((n=z.read(b))!=-1){if(a.cancelled.get())throw new IOException("cancelled");o.write(b,0,n);}} z.closeEntry();
                }
