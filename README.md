...
The release build is signed with a **self-signed key** (`app/release.keystore`,
password `mediacompressor` for both store and key) so the CI-built APK
installs directly on a device with no extra setup. This is fine for personal/
offline use, but:

- Every install must come from a build signed with the *same* keystore, or
  Android will refuse to update over the old install (uninstall first if you
  switch keystores).
- If you ever publish this app or hand it to other people, generate your own
  private keystore (`keytool -genkeypair ...`), keep it **out of the repo**,
  and wire it in via GitHub Actions secrets instead of a committed file.
