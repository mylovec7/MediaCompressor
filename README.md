...
## DEWA ULTRA V5 runtime policy
- Native Android MediaCodec/MediaExtractor/MediaMetadataRetriever only; no FFmpeg/Media3 payload.
- H.264 is the hard fallback when a requested HEVC encoder cannot be configured.
- Input decoding follows device-supported Android codecs; output video is MP4/H.264 or HEVC when safely available.
- Reverse is video-only by design; audio is not falsely copied because raw AAC packets cannot be reversed without audio decode/re-encode.
- 1080p working-set ceiling limits RAM/battery pressure for 4K/8K input.
